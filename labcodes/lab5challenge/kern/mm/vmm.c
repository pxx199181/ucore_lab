...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
   ...
   if (*ptep == NULL) {
   ...
   else if (!(*ptep & PTE_U)  && (error_code & 0x004)) {
        ...
        if (page_ref(page) == 1) {
            *ptep |= PTE_W;
            ...
            {
                cprintf("%x, %x, %x, %x,", *(int*)src_kvaddr, *(int*)(src_kvaddr + 4), *(int*)(src_kvaddr + 8), *(int*)(src_kvaddr + 12));
            }
        }
        else {
            ...
            uintptr_t src_kvaddr = page2kva(page);
            uintptr_t dst_kvaddr = page2kva(newPage);

...
alloc_proc(void) {
    cprintf("in func alloc_proc\n");
    //struct proc_struct *proc = kmalloc(sizeof(struct proc_struct));
    struct proc_struct *proc = mem_alloc(sizeof(struct proc_struct));
    if (proc != NULL) {
...
setup_kstack(struct proc_struct *proc) {
    ...
    */
    ...
    proc->kstack = mem_alloc(KSTACKPAGE);
    if (proc->kstack != NULL) {
        return 0;
    }
    return -E_NO_MEM;
...
put_kstack(struct proc_struct *proc) {
    ...
    mem_free((void *)(proc->kstack), KSTACKPAGE);
}
...
bad_fork_cleanup_proc:
    //kfree(proc);
    mem_free(proc, sizeof(struct proc_struct));
    goto fork_out;
}
...
proc_init(void) {
    ...
    /*
    int pid_tmp= kernel_thread(test_func, "pxx_func", 0);
    struct proc_struct *t_proc = find_proc(pid_tmp);
    set_proc_name(t_proc, "pxx_func");
